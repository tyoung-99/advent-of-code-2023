...
def part_2(puzzle):
    def out_of_bounds(beam, energized):
        if 0 <= beam["y"] < len(energized) and 0 <= beam["x"] < len(energized[0]):
            return False
        return True
    ...
    def traverse(beam, energized):
        if out_of_bounds(beam, energized):
            return
        ...
        beam, new_beam = get_new_beams(beam, tile_type)
        traverse(beam, energized)
        if new_beam:
            traverse(new_beam, energized)

    def traverse_helper(beam):
        energized = [
            [
                {
                    "pos_x": False,
        ...
        traverse(beam, energized)
        energized_counts.append(0)
        for line in energized:
            for tile in line:
                energized = "."
                if any(tile[dir] for dir in tile):
                    energized = "#"
                    energized_counts[-1] += 1
        #         print(energized, end="")
        #     print()
        # print()
    ...
    for y, _ in enumerate(puzzle):
        for x, _ in enumerate(puzzle[-1]):
            if x == 0:
                traverse_helper({"x": x, "y": y, "x_change": 1, "y_change": 0})
            if x == len(puzzle) - 1:
                traverse_helper({"x": x, "y": y, "x_change": -1, "y_change": 0})
            if y == 0:
                traverse_helper({"x": x, "y": y, "x_change": 0, "y_change": 1})
            if y == len(puzzle[0]) - 1:
                traverse_helper({"x": x, "y": y, "x_change": 0, "y_change": -1})

    # print(energized_counts)
    print(f"Tiles energized: {max(energized_counts)}")
